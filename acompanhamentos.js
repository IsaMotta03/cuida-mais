...
function criarItemNota(nota) {
  ...
  li.querySelector(".nota-card__remover").addEventListener("click", () =>
    removerNota(nota.id),
  );
...
function inicializarNotas() {
  document
    .getElementById("btnAdicionarNota")
...
function criarItemCheck(check) {
  ...
  li.querySelector(".check-item__input").addEventListener("change", (e) =>
    alternarCheck(check.id, e.target.checked),
  );
  li.querySelector(".check-item__btn-remover").addEventListener("click", () =>
    removerCheck(check.id),
  );
...
function inicializarChecklist() {
  document
    .getElementById("btnAdicionarCheck")
    .addEventListener("click", abrirModalCheck);
  document
    .getElementById("btnFecharCheck")
    .addEventListener("click", () => fecharModal("modalCheck"));
  document
    .getElementById("btnSalvarCheck")
...
function calcularEExibirIMC() {
  ...
  if (!idoso || !idoso.peso || !idoso.altura) {
    ...
    document.getElementById("relatorioClassificacao").textContent =
      "Dados incompletos";
    return;
  }
...
function inicializarSelecaoHumor() {
  ...
  if (humorSalvo) {
    const btnAtivo = document.querySelector(
      `.btn-humor-card[data-humor="${humorSalvo}"]`,
    );
    if (btnAtivo) btnAtivo.classList.add(`ativo-${humorSalvo.toLowerCase()}`);
  }

  botoesHumor.forEach((btn) => {
    btn.addEventListener("click", () => {
      botoesHumor.forEach(
        (b) => (b.className = b.className.replace(/ativo-\S+/g, "").trim()),
      );
      const humor = btn.getAttribute("data-humor").toLowerCase();
      btn.classList.add(`ativo-${humor}`);
      salvarStorage("relatorio_humor_atual", humor);
    });
  });
}
...
function inicializarAlteracaoStatusSaude() {
  ...
  if (dadosSalvos) {
    ...
    if (dadosSalvos.temperatura)
      inputTemperatura.value = dadosSalvos.temperatura;
  }
